...
# Player name mapping to fix mismatches between NBA API and odds data
# Generated by identify_missing_odds_dates.py on 2025-10-30
NAME_MAPPING = {
...
def match_predictions_to_betting_lines(predictions_df: pd.DataFrame,
    ...
    # Apply name mapping to fix known mismatches
    predictions_df = predictions_df.copy()
    if 'player_name' in predictions_df.columns:
        original_names = predictions_df['player_name'].copy()
        predictions_df['player_name'] = predictions_df['player_name'].replace(NAME_MAPPING)

        # Log mappings applied
        mapped_count = (original_names != predictions_df['player_name']).sum()
        if mapped_count > 0:
            logger.debug(f"  Applied name mapping to {mapped_count} predictions")
